...
def Backtrack(S,start_node,goal_node,map_canvas):
    ...
    while(len(navigation)>0):
        ...
        map_canvas[path_node[0]][path_node[1]] = [184,13,218]
        out.write(map_canvas)
...
# draw the hexagon on the map_canvas
    cv2.fillPoly(map_canvas,[np.array(vertices_bloat)], clearance_color)
...
# show the map_canvas
    ...
    while(1):
        ...
        if(int(x1) > 1200 or int(y1)>500 or int(x2) > 1200 or int(y2)>500):
            print("enter the coordinates within the framework (1200,500)")
    ...
    # Find the path using Dijkstra Algorithm# import libraries
import cv2
import numpy as np
import heapq as hq
import copy
...
# show the map_canvas
    ...
    print("Total execution time: ", int(end - start_time))
